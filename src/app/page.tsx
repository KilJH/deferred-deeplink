...
export default function TestPage() {
  const [path, setPath] = useState('/product/123');
  const [params, setParams] = useState('{"id": "abc", "ref": "test"}');
  const [generatedLink, setGeneratedLink] = useState('');
  const [storedLinks, setStoredLinks] = useState<DeferredLink[]>([]);
  const [baseUrl, setBaseUrl] = useState('');

  useEffect(() => {
    // 현재 도메인 기반으로 base URL 설정
  ...
  const generateLink = () => {
    try {
      ...
      if (paramsObj) {
        queryParams.set('params', JSON.stringify(paramsObj));
      }
      const link = `${baseUrl}/link?${queryParams.toString()}`;
